...
const Dashboard = () => {
  ...
  const[sendmessage,setsendmessage] =useState({convoid:null,recid: null,
    message:"",name:"",email:"",});
...
// console.log("user datta",user);
  return (
          ...
          return (
            <div key={index}>
              <div className="grid grid-cols-4 gap-2 m-3">
                ...
                >
                  <div className="font-bold text-gray-800">{usersdata.name}</div>
    ...
    <div className="h-screen border w-[60%] flex flex-col">
      ...
      <div className="h-[75%] bg-gray-300 overflow-scroll">
        {message.length > 0 ? (
      ...
      <div className="w-full flex items-center">
        ...
        <img
          ...
          onClick={() => handleSendMessage(sendmessage.convoid, sendmessage.recid, sendmessage.message)}
        />
      </div>
    </div>
    ...
    //   <div className=" bg-slate-500 h-screen border w-[20%]">
    //     <div className="cursor-pointer grid grid-cols-6 gap-2 ml-3 mt-3 h-[13%]">
    //       <img
    //         src={Avatar}
    //         alt="avatar"
    //         className="col-span-2 w-[100px] h-[70px] rounded-full p-1"
    ...
    //                 }}
    //               >
    //                 <div className="font-bold">{usersdata.name}</div>
    ...
    //   {/* this is middle part */}
    //   <div className=" h-screen border w-[60%] flex flex-col">
    //     <div className=" cursor-pointer grid grid-cols-12 gap-1 h-[13%] p-3 border border-gray-950  rounded-e-md bg-slate-500 items-center ">
    //       <img
    //         src={Avatar}
    //         alt="avatar"
    //         className="col-span-2 w-[70px] h-[70px] rounded-full p-1"
    //       />
    //       <div className="col-span-3">
    //         <div className="font-bold">{sendmessage.name}</div>
    //         <h1>{sendmessage.email}</h1>
    //       </div>
    //       <div className="col-end-13 col-span-2">
    //         <img src={callicon} alt="call" className="w-[30px] h-[30px] m-2" />
    //       </div>
    ...
    //     <div className="h-[75%] bg-chatback overflow-scroll">
    //       {message.length>0 ? (message.map((message, index) => {
    //         return (
    //           <div key={index}>
    //             {user._id === message.userd._id ? (
    //               <div className="bg-primary max-w-[40%] border px-2 py-2 mt-5 ml-auto mr-4">
    //                 <p className="break-words">{message.message}</p>
    //               </div>
    //             ) : (
    //               <div className=" bg-chatrecive max-w-[40%] border px-2 py-2 mt-5 ml-4">
    //                 <p className="break-words">{message.message}</p>
    //               </div>
    //             )}
    //           </div>
    ...
    //         }}
    //       />
    //       <img src={sendicon} alt="sendicon" className=" cursor-pointer w-[30px] h-6 ml-3 " onClick= {()=>handleSendMessage(sendmessage.convoid,sendmessage.recid,sendmessage.message)} />
    //     </div>
    //   </div>
    ...
    //   <div className="h-screen border w-[20%] ">
    //         {availableusers.length>0 ? (availableusers.map((user,index)=>{
    //           return(
    //             <div key={index}>
    //             <div className="grid grid-cols-4 gap-2 m-3">
    //               <img
